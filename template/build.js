 ...
 *                       template source hash, imported by the CLI so the
...
export async function buildDist() {
  const sourceHash = templateSourceHash();
  ...
  writeFileSync(
    ...
    [
      "// AUTO-GENERATED by template/build.js — do not edit.",
      `export const defaultRendererSourceHash = ${JSON.stringify(sourceHash)};`,
      `export const defaultRendererHtml = ${JSON.stringify(html)};`,
      "export default defaultRendererHtml;",
      "",
    ].join("\n"),
  );
  console.log(
    `Built template/dist/index.html + template/dist/shell.js + shared renderer (${(html.length / 1024).toFixed(1)} KB)`,
  );
  return html;
}
