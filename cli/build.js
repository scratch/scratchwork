#!/usr/bin/env bun
/*
 * Build the CLI into a standalone binary: cli/dist/scratch.
 *
 *   1. Build the renderer (template/dist/index.html + template/dist/shell.js).
 *   2. `bun build --compile` scratch.js into a single self-contained executable.
 *      scratch.js imports ../template/dist/shell.js (a literal dynamic import),
...
const OUT_BIN = join(OUT_DIR, "scratch");

// 1. Build the renderer shell the binary will embed.
await buildDist();

// 2. Compile scratch.js (which imports ../template/dist/shell.js) into a binary.
mkdirSync(OUT_DIR, { recursive: true });
const proc = Bun.spawnSync(
  ["bun", "build", join(here, "scratch.js"), "--compile", "--outfile", OUT_BIN],
...
console.log(`Built cli/dist/scratch (standalone binary, renderer shell embedded)`);
