---
# Basic metadata
title: "Scratchwork - Documentation"
description: "Complete documentation for Scratchwork - the zero-config MDX static site builder"
keywords: ["MDX", "static site", "React", "Bun", "markdown", "documentation"]
author: "Pete Koomen"
robots: "index, follow"
lang: "en"
canonical: "https://scratchwork.dev/docs/"

# Open Graph (social sharing)
image: "https://scratchwork.dev/scratchwork-social.png"
url: "https://scratchwork.dev/docs/"
type: "website"
siteName: "Scratchwork"
locale: "en_US"
siteUrl: "https://scratchwork.dev"
---
...
Scratchwork is two executable programs that can be used together to create and publish static websites with Markdown and React:

The [Scratchwork CLI](#scratch-cli) is a command line utility for scaffolding, building, and publishing [Scratchwork projects](#scratch-projects) as static websites.

The [Scratchwork server](#scratch-server) is a server for serving static websites built with the Scratchwork CLI. There's a public server hosted on [scratchwork.dev](https://scratchwork.dev) and you can host your own on Cloudflare.
...
Scratchwork uses a simple project structure to avoid the need for boilerplate and configuration:
...
my-site/
  ...
  .scratchwork/ (collapsed)    # Project config
...
Scratchwork uses [mdx](https://mdxjs.com/) to translate markdown elements into React components. You can customize the default components used in this translation by adding new ones to `src/markdown/`
...
The page wrapper component (`your-project/src/template/PageWrapper.tsx`) is used in Scratchwork's html template to wrap your pages. It is the only component that the [build process](#build-pipeline) expects to exist.
...
Static files can also live in `pages/` next to your content. Scratchwork will automatically translate relative file system paths like this...
...
Scratchwork uses Tailwind CSS. Edit `src/tailwind.css` to change the styling of your pages:
...
The CLI builds your project and publishes it to a Scratchwork server. Install it with:
...
Deletes `dist/`, `.scratchwork/cache/`, and `.scratchwork/dev/`.
...
For more on servers and URLs, see [Scratchwork Server](#scratch-server).

#### scratch login

Log in to a Scratchwork server:

```bash
scratch login                     # Default: app.scratchwork.dev
scratch login https://my-server.com
```

The CLI shows a verification code and opens your browser. Sign in and approve the device. Credentials are saved to `~/.scratchwork/credentials.json`.
...
Updates `.scratchwork/project.toml` with server URL, project name, and visibility.
...
```bash
scratch tokens use scratchwork_abc123...
```
...
Token resolution priority: `SCRATCHWORK_TOKEN` env var → `.env` file → `~/.scratchwork/credentials.json`
...
**Build cache** lives in `.scratchwork/cache/`. Clear it with `scratch clean`.
...
A Scratchwork server lets you share your work with others. Publish privately to share with specific people or your team, or make it public for anyone to see.

The server runs on Cloudflare (Workers, D1, and R2). You can use the hosted version on scratchwork.dev or run your own.

### scratchwork.dev

We host a public Scratchwork server instance on [scratchwork.dev](https://scratchwork.dev) Anyone can publish public or private projects for free while Scratchwork is in Preview.

**Note:** Projects on scratchwork.dev expire after 30 days during the Preview period.

When you publish, your site is available at either of the following:

```
https://pages.scratchwork.dev/<your-email>/<project-name>/
https://pages.scratchwork.dev/<your-user-id>/<project-name>/
```

For example, if you're `pete@example.com` and your project is `my-blog`:

```
https://pages.scratchwork.dev/pete@example.com/my-blog/
https://pages.scratchwork.dev/abc123/my-blog/
```

Self-hosted Scratchwork servers that are restricted to accounts on @example.com support simpler routing:

```
https://pages.mydomain.com/pete/my-blog/
```

**Preview warning:** Scratchwork is in preview. Don't use it for anything sensitive or critical. The code is open source--if you need more security, self-host behind Cloudflare Access.
...
Or in `.scratchwork/project.toml`:
...
Run your own Scratchwork server when you need:
...
1. Clone the repo:
```bash
git clone https://github.com/scratchwork/scratchwork
cd scratchwork
```
...
Scratchwork uses two subdomains:
...
```bash
# Run any needed migrations on your Scratchwork Server database
bun ops server -i <server-name> db migrate

# Deploy the scratchwork server
bun ops server -i <server-name> deploy
```

#### Publishing to www and root domain

You can configure your Scratchwork server to serve a specific project on the root domain and `www` subdomain:

1. Deploy a Scratchwork server without setting this value
...
Scratchwork separates the app and content subdomains for security. The app handles authentication and API requests. Content serves user-uploaded files.
...
**Preview warning:** Scratchwork is new and hasn't been audited. Don't rely on local authentication for sensitive content. If you need stronger security, self-host behind Cloudflare Access.
...
```bash
curl -H "Authorization: Bearer <token>" https://app.scratchwork.dev/api/me
```

Get your token from `~/.scratchwork/credentials.json` after running `scratch login`.
...
curl -X POST -H "Authorization: Bearer <token>" \
  ...
  -d '{"name": "my-project"}' \
  https://app.scratchwork.dev/api/projects
```
...
curl -X PATCH -H "Authorization: Bearer <token>" \
  ...
  https://app.scratchwork.dev/api/projects/my-blog
```
...
curl -X POST -H "Authorization: Bearer <token>" \
  ...
  "https://app.scratchwork.dev/api/projects/my-blog/deploy?visibility=public"
```
...
curl -X POST -H "Authorization: Bearer <token>" \
  ...
  https://app.scratchwork.dev/api/projects/my-blog/share-tokens
```
...
If you run into an issue with Scratchwork, point your favorite coding agent at [our repo](https://github.com/scratchwork/scratchwork) and ask it to figure out what's going on. Feel free to open an issue with a markdown plan for fixing the problem. If your plan makes sense we'll implement it.
