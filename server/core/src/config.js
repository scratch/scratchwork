...
import { isReservedSlug, isSafeProjectIdentifier, normalizeAccessGroup, safeDomain } from "./access.js";
...
export function makeServerConfigLayer(env) {
...
export function readServerConfig(env) {
    return Effect.gen(function* () {
        ...
        const appUrl = yield* readPublicUrl(env.SCRATCHWORK_APP_URL || urlFromDomain(env.SCRATCHWORK_APP_DOMAIN), "SCRATCHWORK_APP_URL");
        const contentUrl = yield* readPublicUrl(env.SCRATCHWORK_CONTENT_URL || urlFromDomain(env.SCRATCHWORK_CONTENT_DOMAIN), "SCRATCHWORK_CONTENT_URL");
...
function readPublicUrl(value, name) {
    if (value == null || value === "")
...
function readHomepage(env, appUrl, contentUrl) {
    return Effect.gen(function* () {
        ...
        if (domains == null && project == null)
            return { homepageUrls: [] };
        if (domains == null || project == null) {
            return yield* Effect.fail(new ServerConfigError({
        ...
        for (const item of csvItems(domains)) {
            ...
            if (url == null || homepageUrls.includes(url))
                continue;
            if (url === appUrl || url === contentUrl) {
                return yield* Effect.fail(invalidValue("SCRATCHWORK_HOMEPAGE_DOMAINS", item, "a homepage origin distinct from the app and content origins, which use path-based routing"));
            }
            homepageUrls.push(url);
        }
        if (homepageUrls.length === 0) {
            return yield* Effect.fail(invalidValue("SCRATCHWORK_HOMEPAGE_DOMAINS", domains, 'a comma-separated list of domains or origins, like "example.com,www.example.com"'));
        }
...
function readAuthConfig(env, appUrl) {
    return Effect.gen(function* () {
...
function readLocalOAuthEndpoints(env, appUrl) {
    const names = [
    ...
    if (appUrl == null || !isLiteralLoopbackHost(new URL(appUrl).hostname)) {
        return Effect.fail(new ServerConfigError({
            message: "SCRATCHWORK_LOCAL_OAUTH_* endpoints are accepted only when SCRATCHWORK_APP_URL uses a loopback host",
        }));
    }
    if (values.some((value) => value == null)) {
        return Effect.fail(new ServerConfigError({ message: `Set all of ${names.join(", ")} or none of them` }));
    }
    for (const [index, value] of values.entries()) {
        if (!isLoopbackUrl(value)) {
            return Effect.fail(invalidValue(names[index], value, 'a loopback URL, like "http://127.0.0.1:4300/authorize"'));
        }
    }
    const [authorizeUrl, tokenUrl, jwksUrl] = values;
...
function readCloudflareAccessConfig(env, appUrl) {
    return Effect.gen(function* () {
        ...
        if (teamDomain == null) {
            return yield* Effect.fail(invalidValue("SCRATCHWORK_CF_ACCESS_TEAM_DOMAIN", teamDomainValue, 'a Cloudflare Access team domain, like "myteam" or "myteam.cloudflareaccess.com"'));
        }
        return {
...
const LocalJwksSchema = Schema.parseJson(Schema.Struct({
    keys: Schema.Array(Schema.Object).pipe(Schema.filter((keys) => keys.length > 0 || "keys must not be empty")),
}));
...
function readLocalCloudflareJwks(value, appUrl) {
    if (value == null || value === "")
        return Effect.succeed({});
    ...
    return Schema.decodeUnknown(LocalJwksSchema)(value).pipe(Effect.map((parsed) => ({ localJwks: parsed.keys })), Effect.mapError(() => invalidValue("SCRATCHWORK_LOCAL_CF_ACCESS_JWKS", value, "a generated JWKS JSON document with at least one public key")));
}
...
function readCommonAuthConfig(env, sessionSecret) {
    return Effect.gen(function* () {
        ...
        return {
            ...
            sessionTtlSeconds: yield* readPositiveInteger(env.SCRATCHWORK_AUTH_SESSION_SECONDS, 60 * 60 * 24 * 30, "SCRATCHWORK_AUTH_SESSION_SECONDS"),
        };
    });
}
...
function rejectRetiredEnvVars(env) {
...
function readAccessGroup(value, fallback, name) {
    return normalizeAccessGroup(value == null || value === "" ? fallback : value).pipe(Effect.mapError((cause) => new ServerConfigError({ message: `${name}: ${cause.message}` })));
}
...
function readDomainSet(value, name) {
