...
export function normalizeAccessGroup(group) {
...
export function accessGroupMatches(group, principal) {
    const terms = parseAccessGroup(group);
    if (terms == null)
        return false;
...
export function accessGroupModify(group, changes) {
...
export function accessGroupTerms(group) {
    const terms = parseAccessGroup(group);
    if (terms == null)
        return [];
    return terms
...
export function accessGroupUsesOnlyDomains(group, domains) {
    if (domains.size === 0)
        return true;
    const terms = parseAccessGroup(group);
    if (terms == null)
...
function explainInvalidAccessGroup(group) {
...
function parseAccessGroup(group) {
    ...
    if (terms.some((term) => term._tag === "Public") && terms.length > 1)
        return null;
    if (terms.some((term) => term._tag === "Private") && terms.length > 1)
        return null;
    return dedupeTerms(terms);
}
/** Returns true when the principal satisfies one term. */
function termMatches(term, principal) {
    if (term._tag === "Public")
        return true;
    if (term._tag === "Private" || principal == null)
        return false;
    const email = principal.email.toLowerCase();
    if (term._tag === "Email")
        return email === term.email;
...
function serializeTerms(terms) {
    if (terms.length === 1 && terms[0]._tag === "Public")
        return "public";
    if (terms.length === 1 && terms[0]._tag === "Private")
        return "private";
    return terms.map((term) => term._tag === "Email"
        ? term.email
        : term._tag === "Domain"
            ? `@${term.domain}`
            : term._tag.toLowerCase()).join(",");
}
