...
interface LoadDeployEnvOptions {
  ...
  readonly loadDefaultEnvFiles?: boolean;
...
export async function loadDeployEnv(options: LoadDeployEnvOptions): Promise<LoadedDeployEnv> {
  ...
  const candidates = options.loadDefaultEnvFiles === false
    ? []
    : [
        join(serverRoot, ".env"),
  ...
  if (explicitEnvFile != null) {
    const explicitRoots = options.explicitEnvRoots ?? [process.cwd(), packageRoot, serverRoot, repoRoot];
    const explicit = await readExplicitEnvFile(explicitEnvFile, explicitRoots);
