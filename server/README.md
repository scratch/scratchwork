...
- Add a more-specific Access application for `<app-domain>/auth/cli/token` with a **Bypass / Everyone** policy. This is the PKCE-protected, one-time code-exchange endpoint: it intentionally requires no pre-existing Access credential, while the Worker still validates the signed code, exact loopback redirect, verifier, expiry, and replay record. Without this narrow bypass, a first login cannot acquire the Access JWT it would need to pass the exchange request through the edge. Do not bypass other `/auth/*` paths.
- `scratchwork login` otherwise works unchanged: the browser passes Access, and the server converts the asserted identity into the CLI's bearer token. The browser returns a short-lived authorization code whose payload contains only an encrypted form of the verified Access JWT; the bypassed back-channel exchange decrypts it. The CLI stores the returned JWT and sends it back as a `cf-access-token` header on every later API request.
