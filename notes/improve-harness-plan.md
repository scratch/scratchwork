...
`[~]` Test OAuth/OIDC in three complementary layers *(hermetic provider done — `e2e/src/oauth-provider.ts` + the loopback-gated `SCRATCHWORK_LOCAL_OAUTH_*` config; conformance lane implemented and run — `bun run conformance` in `e2e/` drives the basic RP plan against the pinned suite, scoped by decision (Pete, 2026-07-20, see `notes/oidc-conformance-spike.md`) to the seven tampered-token-rejection modules, all green; the plan's login-completion modules are N/A by design — Scratchwork's RP deliberately consumes only the ID token, no userinfo — narrowing "conformance coverage" from certification-green to security-modules-green; remaining step is wiring the lane into `bun run ci` once timed; real-provider smoke is a manual pre-release step by design)*:
