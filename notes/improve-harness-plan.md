...
`[~]` Test OAuth/OIDC in three complementary layers *(hermetic provider done — `e2e/src/oauth-provider.ts` + the loopback-gated `SCRATCHWORK_LOCAL_OAUTH_*` config; conformance lane implemented and run — `bun run conformance` in `e2e/` drives the full basic RP plan against the pinned suite, 8/14 modules green; the six remaining need a product decision (userinfo support + email-claim sourcing + single-key kid relaxation) recorded in `notes/oidc-conformance-spike.md` before the lane can join `bun run ci`; real-provider smoke is a manual pre-release step by design)*:
